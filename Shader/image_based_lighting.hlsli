...
{
    return lut_ggx.Sample(samplerStates[LINEAR], brdf_sample_point);
}
float4 sample_skybox(float3 v, float roughness)
{
    ...
    return skybox.SampleLevel(samplerStates[LINEAR], sample_point, lod);
}
float4 sample_diffuse_iem(float3 v)
{
    return diffuse_iem.Sample(samplerStates[LINEAR], v);
}
float4 sample_specular_pmrem(float3 v, float roughness)
{
    ...
    return specular_pmrem.SampleLevel(samplerStates[LINEAR], v, lod);
}
