...
float3 f_schlick(float3 f0, float3 f99, float VoH)
{
    return f0 + (f99 - f0) * pow(clamp(1.0 - VoH, 0.0, 1.0), 5.0);
}
...
float3 ibl_radiance_lambertian(float3 N, float3 V, float roughness, float3 diffuse_color, float3 f0)
{
    ...
    float2 brdf_sampler_point = clamp(float2(NoV, roughness), 0.0, 1.0);
    float2 f_ab = sample_lut_ggx(brdf_sampler_point).rg;
