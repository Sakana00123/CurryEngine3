...
VS_OUT main(VS_IN vin)
{
    ...
    //vout.world_normal.xyz = normalize(mul(vin.normal, inverse_transpose_world).xyz);
    vout.world_normal.w = 0;

    vin.tangent.w = 0;
    vout.world_tangent = normalize(mul(vin.tangent, world));
    //vout.world_tangent.xyz = normalize(mul(vin.tangent, inverse_transpose_world)).xyz;
